 ...
 *
 * Reads doctor-visible check sources via a literal-string scan, enumerates
...
function enumerateCheckNames(): Set<string> {
  const source = [DOCTOR_TS_PATH, ONBOARD_CHECKS_TS_PATH]
...
describe('doctor-categories drift guard', () => {
  ...
  test('every categorized name is currently used in doctor-visible sources (no stale entries)', () => {
    const discovered = enumerateCheckNames();
    const allCategorized = new Set<string>([
      ...BRAIN_CHECK_NAMES,
      ...SKILL_CHECK_NAMES,
    ...
    if (stale.length > 2) {
      throw new Error(
        `These categorized names no longer appear in doctor-visible sources: ${stale.sort().join(', ')}. ` +
